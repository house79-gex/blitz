...
-- Regole braccio, arricchite con arm_class e astina_len
CREATE TABLE IF NOT EXISTS hw_arm_rule (
    ...
    arm_name     TEXT
    -- colonne nuove gestite via migrazione: arm_class TEXT, astina_len REAL
);
-- Formula astina legacy (resta disponibile se usi approccio semplificato)
...
CREATE TABLE IF NOT EXISTS hw_mech_part (
    ...
    formula       TEXT NOT NULL,      -- usa H,L,handle_offset,arm_code,arm_class,arm_len,...
    UNIQUE(mechanism_code, part_key)
);

-- Opzioni ferramenta tipologia, ora includono il meccanismo
...
class TypologiesStore:
    ...
    def _migrate_schema(self):
        # Aggiunge colonne se mancano (SQLite moderno supporta IF NOT EXISTS)
    ...
    def list_typologies(self) -> List[Dict[str, Any]]:
        cur = self._conn.execute("SELECT id, name, category, material, pezzi_totali, updated_at FROM typology ORDER BY name ASC")
        return [
            {"id": r[0], "name": r[1], "category": r[2], "material": r[3], "pezzi_totali": r[4], "updated_at": r[5]}
    ...
    def get_typology_full(self, typology_id: int) -> Optional[Dict[str, Any]]:
        ...
        vars_map = {n: float(v) for (n, v) in self._conn.execute(
            "SELECT name, value FROM typology_var WHERE typology_id=? ORDER BY id", (typology_id,))}
        opts = {k: v for (k, v) in self._conn.execute(
        ...
        for r in self._conn.execute(
            ...
            "FROM typology_component WHERE typology_id=? ORDER BY ord, id", (typology_id,)):
            comps.append({
    ...
    def update_typology(self, typology_id: int, data: Dict[str, Any]) -> None:
        ...
        self._conn.execute(
            ...
            (data.get("nome",""), data.get("categoria",""), data.get("materiale",""),
             (data.get("riferimento_quota") or "esterna"), float(data.get("extra_detrazione_mm") or 0.0),
             int(data.get("pezzi_totali") or 1), data.get("note",""), ts, typology_id))
    ...
    def _maybe_seed(self):
        if int(self._conn.execute("SELECT COUNT(*) FROM hw_brand").fetchone()[0]) == 0:
            ...
            # Arm rules con classi tipo0/1/2 e astina_len dimostrativa
            with contextlib.suppress(Exception):
                self._conn.execute("ALTER TABLE hw_arm_rule ADD COLUMN arm_class TEXT")
            with contextlib.suppress(Exception):
                self._conn.execute("ALTER TABLE hw_arm_rule ADD COLUMN astina_len REAL")
