...
-- Tipologie (minimo necessario per questo contesto; lascia intatto il tuo schema reale)
...
-- NUOVO: Formule multiple per gruppi (nessun override per-opzione)
CREATE TABLE IF NOT EXISTS typology_multi_formula (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
    typology_id  INTEGER NOT NULL REFERENCES typology(id) ON DELETE CASCADE,
    group_name   TEXT NOT NULL,
    label        TEXT NOT NULL,    -- tipicamente uguale al nome del componente (es. "Astina superiore")
...
class TypologiesStore:
    ...
    def _migrate_schema(self):
        # se servono aggiunte future, gestiscile qui
        self._conn.commit()
    ...
    def get_typology_full(self, typology_id: int) -> Optional[Dict[str, Any]]:
        ...
        for c in self._conn.execute(
            ...
            comps.append({
                "id_riga": c[0] or "", "nome": c[1] or "", "profilo_nome": c[2] or "",
    ...
    def create_typology(self, data: Dict[str, Any]) -> int:
        ...
        for idx, c in enumerate(data.get("componenti") or []):
            self._conn.execute(
                ...
                (
                    typ_id, idx,
                    c.get("id_riga",""), c.get("nome",""), c.get("profilo_nome",""),
    ...
    # ----------------- Formule multiple (nuove API) -----------------
    def list_multi_formula_groups(self, typology_id: int) -> List[str]:
        cur = self._conn.execute(
            "SELECT DISTINCT group_name FROM typology_multi_formula WHERE typology_id=? ORDER BY group_name ASC",
            (int(typology_id),)
        )
    ...
    def list_multi_formulas(self, typology_id: int, group_name: str) -> List[Dict[str, Any]]:
        cur = self._conn.execute(
            "SELECT label, formula, note FROM typology_multi_formula WHERE typology_id=? AND group_name=? ORDER BY label ASC",
            (int(typology_id), str(group_name))
        )
        return [{"label": r[0], "formula": r[1], "note": r[2] or ""} for r in cur.fetchall()]

    def upsert_multi_formula(self, typology_id: int, group_name: str, label: str, formula: str, note: str = "") -> None:
        # delete+insert per compatibilità
        with contextlib.suppress(Exception):
            self._conn.execute(
        ...
        self._conn.execute(
            "INSERT INTO typology_multi_formula(typology_id, group_name, label, formula, note) VALUES(?,?,?,?,?)",
            (int(typology_id), str(group_name), str(label), str(formula), str(note or ""))
        )
        self._conn.commit()
