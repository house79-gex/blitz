...
def suggest_qcad_paths() -> list[str]:
    ...
    for p in (
        ...
        candidates.append(p)
    # Windows (se usi la Pi con Windows ARM o cross-env)
...
def launch_qcad(qcad_executable: str, dxf_path: Optional[str] = None, workspace_dir: Optional[str] = None) -> subprocess.Popen:
    ...
    # Nota: su alcune build QCAD è uno script .sh: serve la shell
    if exe.suffix == ".sh":
...
def parse_export_json(json_path: str) -> Tuple[Optional[float], Dict[str, Any]]:
    ...
    - Prova chiavi: 'lastDimension' (numero), 'dimensions' (lista di oggetti con 'value'),
      oppure 'entities' con type dimension e 'value'.
    Ritorna (last_dimension_value, full_dict).
    """
    p = Path(json_path)
    if not p.exists():
        return None, {}
        ...
        if isinstance(dims, list):
            for item in dims[::-1]:
                try:
                    ...
                    continue
    # 3) Entità dimension all'interno di entities
    if last_dim is None:
