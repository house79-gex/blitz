...
# Safe expression evaluator: consente solo operatori aritmetici, numeri, variabili, funzioni whitelisted
_ALLOWED_NODES = {
    ...
    ast.IfExp,  # cond ? a : b   -> in Python: a if cond else b
    ast.Constant,
}
...
class _SafeEval(ast.NodeVisitor):
    ...
    def visit_BoolOp(self, node: ast.BoolOp):
        ...
        if isinstance(node.op, ast.And):
            res = True
            for v in values: res = res and bool(v)
            return res
        if isinstance(node.op, ast.Or):
            res = False
            for v in values: res = res or bool(v)
            return res
    ...
    def visit_Call(self, node: ast.Call):
        if not isinstance(node.func, ast.Name):
            raise ValueError("Chiamata funzione non permessa")
        fname = node.func.id
...
def safe_eval(expr: str, env: Dict[str, Any]) -> float:
    try:
        ...
        raise ValueError(f"Errore formula '{expr}': {e}")
...
class ElementDef:
    id: str
    role: str
    profile_var: str               # nome della variabile che contiene il profilo scelto (es. "prof_telaio_fisso")
    qty_expr: str                  # espressione che produce un intero (es. "2")
    length_expr: str               # es. "H + 2*k45_fisso"
    angle_a_expr: str              # es. "45"
    angle_b_expr: str              # es. "45"
    note_expr: str = ""            # opzionale
...
class TypologyDef:
    ...
    derived: Dict[str, str] = field(default_factory=dict)   # variabili derivate -> espressione
...
class ParametricEngine:
    ...
    def evaluate(self, inputs: Dict[str, Any]) -> Tuple[List[Part], Dict[str, Any]]:
        # Costruisci ambiente con default + inputs
        env: Dict[str, Any] = {}
