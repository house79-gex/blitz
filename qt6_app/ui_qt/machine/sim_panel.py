...
class SimulationPanel(QWidget):
    ...
    def __init__(self, machine, parent=None):
        ...
        pr_row = QHBoxLayout()
        btn_lp = QPushButton("Lock Morsa SX")
        btn_lp.clicked.connect(lambda: self.machine.command_set_morse(True, self.machine.right_morse_locked))
        btn_lr = QPushButton("Unlock Morsa SX")
        btn_lr.clicked.connect(lambda: self.machine.command_set_morse(False, self.machine.right_morse_locked))
        pr_row.addWidget(btn_lp); pr_row.addWidget(btn_lr)
        lay.addLayout(pr_row)

        pr_row2 = QHBoxLayout()
        btn_rp = QPushButton("Lock Morsa DX")
        btn_rp.clicked.connect(lambda: self.machine.command_set_morse(self.machine.left_morse_locked, True))
        btn_rr = QPushButton("Unlock Morsa DX")
        btn_rr.clicked.connect(lambda: self.machine.command_set_morse(self.machine.left_morse_locked, False))
