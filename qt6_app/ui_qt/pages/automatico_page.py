# v6 — Sequenza fedele: barra-per-barra, pezzi per barra ordinati per lunghezza decrescente,
# plan_seq congelato; nessun salto tra barre quando "Sequenza stretta per barra" è attiva.
...
class LabelPrinter:
    ...
    def _show_preview_dialog(self, pil_img, title="Anteprima etichetta"):
        try:
            from PySide6.QtWidgets import QDialog, QVBoxLayout, QLabel, QPushButton
            from PySide6.QtGui import QPixmap, QImage
            import numpy as np
            # Converte PIL 1-bit o L in QImage
            if pil_img.mode != "L":
            ...
            lay = QVBoxLayout(dlg)
            lab = QLabel(); lab.setPixmap(pix); lab.setScaledContents(True)
            lay.addWidget(lab)
            btn = QPushButton("Chiudi"); btn.clicked.connect(dlg.accept)
            lay.addWidget(btn)
            dlg.resize(min(800, w+40), min(600, h+80))
            dlg.exec()
        except Exception:
            # Fallback: salva PNG temporaneo
            with contextlib.suppress(Exception):
                import tempfile, os
                p = tempfile.mktemp(suffix=".png")
                pil_img.save(p)
                if self.toast: self.toast(f"Anteprima salvata: {p}", "info")
    ...
    def print_label(self,
        ...
        # Serve PIL per generare l'immagine anche in preview
        if self._pil is None:
            if self.toast: self.toast("Manca 'Pillow': impossibile generare etichetta.", "warn")
            return False
        try:
            ...
            # Se non abilitata o non configurata la stampante → anteprima e stop
            if (not self.enabled) or (self._ql is None) or (not self.printer):
                if self.preview_if_no_printer:
                    self._show_preview_dialog(img, title="Anteprima etichetta (simulazione)")
                if self.toast: self.toast("Simulazione etichetta (stampante non configurata).", "info")
                return True
...
class AutomaticoPage(QWidget):
    ...
    def _build(self):
        ...
        title_lb = QLabel("Etichette"); title_lb.setStyleSheet("font-weight:800; font-size:16px;"); lb.addWidget(title_lb)
    ...
    def _test_label(self):
        piece = {
            "seq_id": 999, "profile":"DEMO", "element":"Montante #1",
    ...
    # Ottimizzazione: congela ordine "barra-per-barra" e ordina ogni barra per L decrescente
    ...
    def _optimize_profile(self, profile: str):
        ...
        # Ordinamento iniziale per stabilità nel packing
        ...
        # ORDINAMENTO pezzi all'interno di ogni barra: lunghezze decrescenti (come richiesto)
        for b in bars:
            try:
    ...
    # Fuori quota / posizionamento (con fix parentesi)
    ...
    def _simulate_cut_once(self):
        ...
        # se non armato/lock, permetti decremento manuale
                ...
                if self._mode == "plan":
                    ...
                    if nxt_piece and current_piece:
                        if self._strict_bar_sequence:
                            # Sempre e solo il prossimo in sequenza: nessun salto barra
                            self._advance_to_next_piece()
                            self._unlock_brake(silent=True)
                        else:
                            # Compat vecchio comportamento (auto-continue su stesso job)
