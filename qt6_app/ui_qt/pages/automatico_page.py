# v5 — Sequenza fedele (barra per barra), meta propagati, multi-template per profilo/elemento, QR opzionale
...
from PySide6.QtGui import (
    QKeySequence, QShortcut, QColor, QBrush, QFont, QKeyEvent, QCursor
)
...
# Resolver etichette: multi-template per profilo/elemento
try:
...
class OptimizationConfigDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.cmb_solver.addItems(["ILP_KNAP", "ILP", "BFD"])
        ...
        self.chk_reversible = QCheckBox("Profilo reversibile")
        ...
        self.chk_auto_cont = QCheckBox("Auto-continue pezzi identici (stessa barra)")
        self.chk_auto_cont.setChecked(auto_cont)

        self.chk_auto_across = QCheckBox("Consenti auto-continue su barra successiva")
        self.chk_auto_across.setChecked(auto_across)

        self.chk_strict_seq = QCheckBox("Sequenza stretta per barra (rispetta grafica)")
        self.chk_strict_seq.setChecked(strict_seq)
    ...
    def _save_and_close(self):
        ...
        def i(txt, d):
            try:
                return int(float((txt or "").replace(",", ".")))
            except Exception:
                return d
...
class LabelPrinter:
    ...
    def print_label(self,
        ...
        if self._ql is None or self._pil is None:
            if self.toast:
                self.toast("Modulo 'brother_ql' o 'Pillow' mancante: etichetta non stampata.", "warn")
            return False
        if not self.printer:
            if self.toast:
                self.toast("Stampante non configurata.", "warn")
            return False
        try:
            ...
            if self.toast:
                self.toast(f"Errore stampa: {e}", "err")
            return False
...
class AutomaticoPage(QWidget):
    def __init__(self, appwin):
        ...
        # Contatori per firma pezzo (per pannello destra)
    ...
    def _build(self):
        ...
        self.banner.setStyleSheet(
            "QLabel { background:#ffe7ba; color:#1b1b1b; font-size:18px; font-weight:800; "
            "padding:8px 12px; border:1px solid #c49a28; border-radius:6px; }"
        )
        ...
        self.tbl_cut.setHorizontalHeaderLabels(
            ["SeqID", "Profilo", "Elemento", "Lunghezza (mm)", "Ang SX", "Ang DX", "Q.tà", "Note"]
        )
        ...
        self.btn_start_row.setStyleSheet(
            "QPushButton { background:#2ecc71; color:white; font-weight:900; font-size:20px; "
        ...
        quota_title.setAlignment(Qt.AlignCenter)
        quota_title.setMinimumHeight(56)
        quota_title.setStyleSheet(
            "QLabel { background:#f0f8ff; color:#2c3e50; font-weight:900; font-size:20px; "
            "padding:12px 24px; border:2px solid #3498db; border-radius:10px; }"
        )
        ...
        self.lbl_quota_card.setMinimumWidth(320)
        ...
        self.lbl_target = QLabel("0")
        self.lbl_target.setStyleSheet(big)
        row1.addWidget(self.lbl_target)
        row1.addStretch(1)

        row2 = QHBoxLayout()
        row2.addWidget(QLabel("Tagliati:"))
        self.lbl_done = QLabel("0")
        self.lbl_done.setStyleSheet(big + "color:#2ecc71;")
        row2.addWidget(self.lbl_done)
        row2.addStretch(1)

        row3 = QHBoxLayout()
        row3.addWidget(QLabel("Rimanenti:"))
        self.lbl_remaining = QLabel("-")
        self.lbl_remaining.setStyleSheet(big + "color:#f39c12;")
        row3.addWidget(self.lbl_remaining)
        row3.addStretch(1)
        ...
        label_box = QFrame()
        label_box.setFrameShape(QFrame.StyledPanel)
        label_box.setStyleSheet("QFrame { border:1px solid #3b4b5a; border-radius:6px; }")
        lb = QVBoxLayout(label_box)
        lb.setContentsMargins(10, 10, 10, 10)
        lb.setSpacing(6)

        title_lb = QLabel("Etichette")
        title_lb.setStyleSheet("font-weight:800; font-size:16px;")
        ...
        btn_label_test = QPushButton("Test etichetta")
        btn_label_test.clicked.connect(self._test_label)
    ...
    def _show_banner(self, msg: str, level: str = "info"):
        ...
        self.banner.setStyleSheet(
            f"QLabel {{{sty} font-size:20px; font-weight:900; padding:10px 14px; border-radius:8px;}}"
        )
    ...
    def _on_label_toggle(self, on: bool):
        ...
        cfg = dict(read_settings())
        cfg["label_enabled"] = self._label_enabled
        write_settings(cfg)
    ...
    def _test_label(self):
        piece = {
            ...
            "ax": 45.0,
            "ad": 0.0,
            "meta": {"commessa": "JOB-2025-001", "element_id": "E-0001", "infisso_id": "W-42", "misura_elem": 1234.5},
        }
    ...
    def _import_cutlist(self):
        ...
        if dlg.exec() and getattr(dlg, "selected_order_id", None):
            ...
            if data.get("type") != "cutlist":
                QMessageBox.information(self, "Importa", "Seleziona un ordine di tipo cutlist.")
                return
    ...
    def _header_items(self, profile: str) -> List[QTableWidgetItem]:
        ...
        seq_it = QTableWidgetItem("")
        seq_it.setFont(font)
        seq_it.setBackground(bg)
        seq_it.setFlags(Qt.ItemIsEnabled)
        items.append(seq_it)

        itp = QTableWidgetItem(profile or "—")
        itp.setFont(font)
        itp.setBackground(bg)
        itp.setForeground(QBrush(Qt.black))
    ...
    def _load_cutlist(self, cuts: List[Dict[str, Any]]):
        ...
        for prof in order:
            ...
            for c in groups[prof]:
                ...
                # Meta completa salvata in UserRole della prima cella
    ...
    def _on_optimize_clicked(self):
        ...
        if not prof:
            QMessageBox.information(self, "Ottimizza", "Seleziona un profilo o importa una lista.")
            return
    ...
    def _open_opt_dialog(self, profile: str):
        ...
        with contextlib.suppress(Exception):
            self._opt_dialog.startRequested.connect(self._handle_start_trigger)
    ...
    def _optimize_profile(self, profile: str):
        ...
        # Crea pezzi atomici mantenendo meta/element
        ...
        if not pieces:
            QMessageBox.information(self, "Ottimizza", f"Nessun pezzo per '{prof}'.")
            return
        ...
        # Ordinamento (stabilità) — non influisce sulla sequenza finale
        ...
        if solver in ("ILP_KNAP", "ILP"):
            ...
            if not bars:
                bars, rem = self._pack_bfd(
                    pieces, stock, kerf_base,
    ...
    def _build_sequential_plan(self):
        # Sequenza fedele: barra per barra, pezzo per pezzo come in grafica
        self._seq_plan.clear()
        seq_id = 1
    ...
    def _pack_bfd(self, pieces: List[Dict[str, Any]], stock: float, kerf_base: float,
        ...
        for p in pieces:
            ...
            placed = False
            for b in bars:
                used = bar_used_length(
                    b, kerf_base, self._ripasso_mm,
                    reversible, thickness_mm,
                    angle_tol, max_angle, max_factor
                )
                extra = joint_consumption(
                    b[-1], kerf_base, self._ripasso_mm,
                    reversible, thickness_mm,
                    angle_tol, max_angle, max_factor
                )[0] if b else 0.0
            ...
            if not placed:
                bars.append([p])
        rem = residuals(
            bars, stock, kerf_base, self._ripasso_mm,
            reversible, thickness_mm,
            angle_tol, max_angle, max_factor
        )
        return bars, rem
    ...
    def _next_seq_piece(self) -> Optional[Dict[str, Any]]:
        nxt = self._seq_pos + 1
        if 0 <= nxt < len(self._seq_plan):
            return self._seq_plan[nxt]
        return None
    ...
    def _enforce_length_limits(self, length_mm: float) -> tuple[float, bool, float, float]:
        ...
        if length_mm < min_q - 1e-6:
            return length_mm, False, min_q, max_q
        if length_mm > max_q + 1e-6:
            return max_q, True, min_q, max_q
        return length_mm, True, min_q, max_q

    def _position_machine_exact(self, target_mm: float, ax: float, ad: float,
                                profile: str, element: str, allow_pressers_locked: bool = False):
        ...
        try:
            ...
            return
        self._move_target_mm = float(target_mm)
        ...
        if not ok and eff_len < min_q - 1e-6:
            ...
            if both_zero and eff_len + self._fq_offset_mm >= min_q - 1e-6:
                self._fq_state = {
                    ...
                    "ax": float(ax), "ad": float(ad),
                    "profile": profile, "element": element, "min_q": float(min_q),
                }
    ...
    def _handle_start_trigger(self):
        if self._mode != "plan" or not self._seq_plan:
            return
    ...
    def _simulate_cut_once(self):
        ...
        # Se non armato/lock, consenti decremento manuale e stampa (modalità manuale/altro)
        ...
        if self._fq_state.get("active", False):
            ...
            if mode == "fq":
                if phase == "intest":
                    ...
                    self._fq_state = {
                        "active": False, "mode": "", "phase": "", "sub": "",
                        "final_target": 0.0, "ax": 0.0, "ad": 0.0,
                        "profile": "", "element": "", "min_q": 0.0
                    }
        ...
        if self._opt_dialog and current_piece:
            with contextlib.suppress(Exception):
                self._opt_dialog.update_after_cut(
                    length_mm=float(current_piece["len"]),
            ...
            def _after_pause():
                if self._mode == "plan":
                    ...
                    if nxt_piece and current_piece:
                        ...
                        needs_fq = self._requires_fq_for_piece(
                            nxt_piece["len"], nxt_piece["ax"], nxt_piece["ad"], nxt_piece["profile"]
                        )
                        same = self._same_job(current_piece, nxt_piece)

                        # Se sequenza stretta: MAI attraversare barre automaticamente
                        if self._strict_bar_sequence:
                            if can_auto and same and (not across) and not needs_fq:
                                self._advance_to_next_piece()
                                self._unlock_brake(silent=True)
                        else:
                            if can_auto and same and ((not across) or self._auto_continue_across_bars) and not needs_fq:
                                self._advance_to_next_piece()
                                self._unlock_brake(silent=True)
                    else:
                        if self._opt_dialog:
                    ...
                    if self._active_row is not None:
                        try:
                            q_now = int(self.tbl_cut.item(self._active_row, 6).text())
                        except Exception:
                            q_now = 0
                        if q_now <= 0:
    ...
    def _on_cell_entered(self, row: int, col: int):
        if row < 0 or self._row_is_header(row) or col != 6:
            return
        it = self.tbl_cut.item(row, 6)
        if not it:
            return
