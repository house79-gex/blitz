...
# Store profili (comune a Utility/Semi-automatico)
try:
...
# Tipologie (progettazione parametrica)
try:
    from ui_qt.pages.typologies_page import TypologiesPage
except Exception:
    TypologiesPage = None
...
class UtilityPage(QWidget):
    ...
    def _build(self):
        ...
        # Aggiungo Tipologie per la progettazione parametrica
        for label in ("Tipologie", "Profili", "QCAD", "Backup", "Configurazione", "Temi"):
        ...
        # Pagine
        self.page_tipologies = TypologiesPage(self.appwin, self.profiles_store) if TypologiesPage else QLabel("Tipologie non disponibili")
        ...
        self.stack.addWidget(self.page_tipologies)  # 0
    ...
    def _open_in_qcad_on_profile(self, dxf_path: str):
        self.stack.setCurrentIndex(2)  # QCAD
    ...
    def on_show(self):
        try:
            ...
            # Tipologie non richiede refresh speciale
        except Exception:
            pass
...
# --------- Sotto-pagine esistenti (Profili/QCAD/Backup/Config/Temi) ---------
# Copiate dall'ultima versione funzionante che abbiamo impostato (QCAD esterno e profili condivisi).
# Per brevità di questa risposta restano invariate rispetto alla tua base attuale,
# tranne l’ordine nel menu (Tipologie aggiunta come prima voce).
...
class ProfilesSubPage(QFrame):
    ...
    def __init__(self, appwin, profiles_store):
        ...
        self._build(); self.reload_profiles()
    def _build(self):
        ...
        tip = QLabel("Suggerimento: seleziona un profilo per la preview DXF (popup)."); tip.setStyleSheet("color:#7f8c8d;"); rl.addWidget(tip, row, 0, 1, 4); row += 1
    ...
    def reload_profiles(self):
        ...
        if not self._profiles_index: self._profiles_index = {"Nessuno": 0.0}
    ...
    def _on_select_profile(self, cur: Optional[QListWidgetItem], prev: Optional[QListWidgetItem]):
        ...
        dxf_path = self._get_profile_dxf_path(name); self._show_profile_preview_ephemeral(name)
    ...
    def _new_profile(self):
        ...
        self.lst_profiles.addItem(QListWidgetItem(name)); self.lst_profiles.setCurrentRow(self.lst_profiles.count() - 1)
        self.edit_prof_name.setText(name); self.edit_prof_th.setText("0.0")
    def _save_profile(self):
        name = (self.edit_prof_name.text() or "").strip(); ths = (self.edit_prof_th.text() or "").strip()
    ...
    def _delete_profile(self):
        name = self._get_selected_name(); ifnot = (not name)
        if ifnot: return
    ...
    def close_preview(self):
        try:
            if self._section_popup: self._section_popup.close(); self._section_popup = None
        except Exception: self._section_popup = None
    ...
    def _show_profile_preview_ephemeral(self, name: str, auto_hide_ms: int = 1200):
        ...
        if bw > 0.0 and bh > 0.0:
            screen = QGuiApplication.primaryScreen(); scr = screen.availableGeometry() if screen else QRect(0,0,1920,1080)
    ...
    def _sort_list_widget(lw: QListWidget):
        texts = [lw.item(i).text() for i in range(lw.count())]; texts.sort(); lw.clear()
        for t in texts: lw.addItem(QListWidgetItem(t))

class QcadSubPage(QFrame):
    def __init__(self, appwin, profiles_store):
        super().__init__()
        self.appwin = appwin; self.profiles = profiles_store
        self._monitor_timer: Optional[QTimer] = None; self._last_mtime: float = 0.0
        self._build()
    def _build(self):
        self.setStyleSheet("QFrame { border: 1px solid #3b4b5a; border-radius: 6px; }")
        root = QVBoxLayout(self); root.setContentsMargins(8, 8, 8, 8); root.setSpacing(8)
        grid = QGridLayout(); grid.setHorizontalSpacing(8); grid.setVerticalSpacing(6); row = 0
        ...
        btn_qcad.clicked.connect(self._browse_qcad); grid.addWidget(btn_qcad, row, 2); row += 1
        grid.addWidget(QLabel("Cartella di lavoro:"), row, 0)
        self.edit_ws = QLineEdit(); grid.addWidget(self.edit_ws, row, 1)
        btn_ws = QToolButton(); btn_ws.setIcon(self.style().standardIcon(QStyle.SP_DirIcon))
        btn_ws.clicked.connect(self._browse_ws); grid.addWidget(btn_ws, row, 2); row += 1
        grid.addWidget(QLabel("Profilo target:"), row, 0)
        self.cmb_profile = QComboBox(); self._reload_profiles_combo()
        grid.addWidget(self.cmb_profile, row, 1, 1, 2); row += 1
        root.addLayout(grid)
        rowb = QHBoxLayout(); rowb.setSpacing(8)
        self.btn_open_blank = QPushButton("Apri QCAD (vuoto)"); self.btn_open_blank.clicked.connect(self._open_qcad_blank); rowb.addWidget(self.btn_open_blank)
        self.btn_open_prof = QPushButton("Apri QCAD su DXF profilo"); self.btn_open_prof.clicked.connect(self._open_qcad_on_profile); rowb.addWidget(self.btn_open_prof)
        self.btn_import = QPushButton("Importa export.blitz.json"); self.btn_import.clicked.connect(self._import_export_now); rowb.addWidget(self.btn_import)
        self.btn_monitor = QPushButton("Avvia monitor"); self.btn_monitor.setCheckable(True); self.btn_monitor.clicked.connect(self._toggle_monitor); rowb.addWidget(self.btn_monitor)
        rowb.addStretch(1); root.addLayout(rowb)
        assoc = QHBoxLayout(); assoc.setSpacing(8)
        self.btn_assoc = QPushButton("Associa DXF al profilo…"); self.btn_assoc.clicked.connect(self._associate_dxf_to_profile); assoc.addWidget(self.btn_assoc)
        self.btn_new_from_dxf = QPushButton("Crea profilo da DXF…"); self.btn_new_from_dxf.clicked.connect(self._create_profile_from_dxf); assoc.addWidget(self.btn_new_from_dxf)
        assoc.addStretch(1); root.addLayout(assoc)
        self.lbl_info = QLabel("Ultima quota: —"); self.lbl_info.setStyleSheet("color:#0a0a0a;"); root.addWidget(self.lbl_info, 0)
        root.addStretch(1)
    ...
    def _qcad_exe(self) -> str: return (self.edit_qcad.text() or "").strip()
    def _workspace(self) -> str: return (self.edit_ws.text() or "").strip()
    ...
    def _open_qcad_on_profile(self):
        prof = (self.cmb_profile.currentText() or "").strip(); dxf_path = None
        if self.profiles and prof and hasattr(self.profiles, "get_profile_shape"):
            try:
                shape = self.profiles.get_profile_shape(prof); dxf_path = shape.get("dxf_path") if shape and shape.get("dxf_path") else None
            except Exception: dxf_path = None
        if not dxf_path:
    ...
    def _toggle_monitor(self, on: bool):
        if on:
            if not self._monitor_timer:
                self._monitor_timer = QTimer(self); self._monitor_timer.setInterval(1500); self._monitor_timer.timeout.connect(self._check_export)
            self._monitor_timer.start(); self.btn_monitor.setText("Ferma monitor")
        else:
            if self._monitor_timer: self._monitor_timer.stop(); self.btn_monitor.setText("Avvia monitor")
    def _export_path(self) -> Optional[str]:
        ws = self._workspace(); 
        if not ws: return None
        p = find_export_file(ws); return str(p) if p else None
    def _check_export(self):
        ...
        try:
            ...
            if st.st_mtime > self._last_mtime:
                self._last_mtime = st.st_mtime; self._import_export_now()
        except Exception: pass
    def _import_export_now(self):
        ...
        if not p:
            ...
            if not p: return
        try: last_dim, data = parse_export_json(p)
        except Exception: last_dim, data = None, {}
    ...
    def _associate_dxf_to_profile(self):
        ...
        if not dxf: return
        bbox = compute_dxf_bbox(dxf); self._store_profile_dxf(prof, dxf, bbox)
    def _create_profile_from_dxf(self):
        dxf, _ = QFileDialog.getOpenFileName(self, "Seleziona DXF", "", "DXF Files (*.dxf);;Tutti i file (*)")
        if not dxf: return
        # Semplice prompt in assenza di un dialog dedicato
        name, ok = QFileDialog.getSaveFileName(self, "Nome profilo (salva per confermare)", "", "Profilo (*)")
        if not ok or not name: return
        name = Path(name).stem
        if self.profiles and hasattr(self.profiles, "upsert_profile"):
            try: self.profiles.upsert_profile(name, 0.0)
            except Exception: pass
        bbox = compute_dxf_bbox(dxf); self._store_profile_dxf(name, dxf, bbox); self._reload_profiles_combo()
    def _store_profile_dxf(self, name: str, dxf_path: str, bbox: Optional[Tuple[float, float]]):
        ...
        if bbox: shape["bbox_w"], shape["bbox_h"] = float(bbox[0]), float(bbox[1])
        for m, args in [("set_profile_shape",(name,shape)), ("set_profile_dxf",(name,dxf_path)), ("upsert_profile_shape",(name,shape)), ("upsert_profile_meta",(name,shape))]:
            if hasattr(self.profiles, m):
                try: getattr(self.profiles, m)(*args); return
                except Exception: pass
        if hasattr(self.profiles, "upsert_profile"):
            try: getattr(self.profiles, "upsert_profile")(name, float(0.0), shape)  # se supporta meta
            except Exception: pass
...
class BackupSubPage(QFrame):
    ...
    def _build(self):
        ...
        row = QHBoxLayout(); btn_make = QPushButton("Crea backup"); btn_restore = QPushButton("Ripristina backup")
        row.addWidget(btn_make); row.addWidget(btn_restore); row.addStretch(1); root.addLayout(row)
...
class ConfigSubPage(QFrame):
    ...
    def _build(self):
        ...
        row = 0; root.addWidget(QLabel("Configurazione"), row, 0, 1, 2, alignment=Qt.AlignLeft); row += 1
...
class ThemesSubPage(QFrame):
    ...
    def _build(self):
        ...
        row = QHBoxLayout(); row.addWidget(QLabel("Seleziona tema:")); btn_light = QPushButton("Chiaro"); btn_dark = QPushButton("Scuro")
        row.addWidget(btn_light); row.addWidget(btn_dark); row.addStretch(1); root.addLayout(row)
