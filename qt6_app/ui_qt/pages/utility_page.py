# AGGIUNTO: sottopagina Etichette (LabelsSubPage) + import store.
# File originale mantenuto, modifiche:
# - import QListWidget mancava? (già presente)
# - menu: aggiunto voce "Etichette"
# - stack: aggiunto page_labels
# - definita classe LabelsSubPage a fondo.
...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QToolButton, QStyle, QComboBox, QCheckBox, QSpinBox, QTableWidget,
    QHeaderView
)
...
try:
    ...
    # Stubs minimi se il file non è ancora stato copiato
...
class ProfilesSubPage(QFrame):
    ...
    # (Contenuto invariato - omesso per brevità, copia esatto dal tuo file corrente)
    # Inserisci qui il codice originale delle classi ProfilesSubPage, QcadSubPage, BackupSubPage, ConfigSubPage, ThemesSubPage
    # Per risparmiare spazio non lo ripeto: mantieni il contenuto che hai già.


# (Per brevità: QcadSubPage, BackupSubPage, ConfigSubPage, ThemesSubPage lasciate come nel file originale già mostrato)
...
class LabelsSubPage(QFrame):
    ...
    Placeholder:
      {profile} {element} {length_mm:.2f} {ang_sx:.1f} {ang_dx:.1f} {seq_id} {timestamp} {qty_remaining}
    """
    ...
    def _reload_assoc_list(self):
        ...
        if not assoc:
            self.lbl_assoc_list.setText("Nessuna")
            return
    ...
    def _duplicate_template(self):
        if not self._current_tmpl_name:
            return
        base = f"{self._current_tmpl_name}_COPY"
        name = base; i = 1
        while get_template(name):
            i += 1; name = f"{base}_{i}"
    ...
    def _delete_template(self):
        ...
        if not delete_template(self._current_tmpl_name):
            QMessageBox.information(self,"Template","Impossibile eliminare (DEFAULT o inesistente).")
            return
    ...
    def _test_print_current(self):
        ...
        for raw in tmpl.get("lines", []):
            try:
                ...
                line = raw
            lines.append(line)
        # Usa label_printer se centralizzato
