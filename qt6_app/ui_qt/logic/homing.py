...
def start_homing(machine, callback=None):
    """
    Simulazione HOMING con sensore induttivo e stop a '0 macchina' (quota minima):
    ...
    Aggiorna machine.position_current durante il movimento.
    """
    try:
    ...
    def _run():
        try:
            ...
            # Abilita condizioni safe di homing
            try:
            ...
            # 1) Avvicinamento verso il sensore (min + 5)
            ...
            pos = min(pos + 10.0, getattr(machine, "max_cut_length", pos + 10.0))
            try:
                setattr(machine, "position_current", pos)
                setattr(machine, "encoder_position", pos)
            except Exception:
            ...
            try:
                ...
                # Freno sbloccato, frizione inserita a fine procedura,
                # come concordato durante la modalita' manuale.
                setattr(machine, "brake_active", False)
                setattr(machine, "clutch_active", True)
