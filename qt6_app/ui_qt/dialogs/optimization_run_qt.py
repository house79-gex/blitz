...
# Import dei calcoli con kerf angolare
from ui_qt.logic.refiner import refine_tail_ilp, pack_bars_knapsack_ilp, _residuals, _sum_angle_kerf_min


class OptimizationSummaryDialog(QDialog):
    """
    Finestra di riepilogo ottimizzazione con tabella per-barra e export CSV/PDF.
    Mostra: n° barra, #pezzi, usato (somma lunghezze + somma kerf angolare dei giunti), residuo, efficienza, dettaglio.
    """
    def __init__(self, parent: QWidget, profile: str,
        ...
        hdr = QLabel(f"Profilo: {profile} — Stock: {self._stock:.0f} mm — Kerf base: {self._kerf:.2f} mm (angolare)")
    ...
    def _fill_table(self):
        ...
        for i, bar in enumerate(self._bars):
            pezzi = len(bar)
            # Usato = somma lunghezze + somma kerf angolare sui giunti (N-1)
            used_len = sum(float(piece.get("len", 0.0)) for piece in bar)
            used_kerf = _sum_angle_kerf_min(bar, self._kerf)
            used = used_len + used_kerf

            # Residuo: usa quello pre-calcolato (già coerente con kerf angolare)
    ...
    def _export_csv(self):
        try:
            ...
            with open(path, "w", newline="", encoding="utf-8") as f:
                ...
                w.writerow([f"Profilo: {self._profile}", f"Stock: {self._stock:.0f} mm", f"Kerf base: {self._kerf:.2f} mm (angolare)"])
    ...
    def _export_pdf(self):
        ...
        try:
            ...
            <div>Stock: {self._stock:.0f} mm — Kerf base: {self._kerf:.2f} mm (angolare) — Generato: {datetime.now():%Y-%m-%d %H:%M:%S}</div>
            <br />
            <table>
                <thead>
                    <tr>
...
class OptimizationRunDialog(QDialog):
    ...
    - Visualizzazione grafica del piano
    - Tabella elementi (sempre visibile)
    - Pulsante "Riepilogo…" con export CSV/PDF
    - F7 = simula taglio, F9 = simula avanzamento

    Novità:
    - Residui ed efficienza calcolati con kerf angolare (kerf_base / cos(|ang|))
    """
    ...
    def _compute_plan_once(self):
        ...
        if solver in ("ILP_KNAP", "ILP"):
            bars, rem = pack_bars_knapsack_ilp(pieces, stock=stock, kerf=kerf_base, per_bar_time_s=per_bar_time)
            if not bars:
        ...
        # Residui ricalcolati con kerf angolare effettivo
