...
class Header(QWidget):
    ...
    def _build(self, title: str):
        ...
        # Titolo centrato (raddoppiato)
        self.lbl_title = QLabel(title, center)
        self.lbl_title.setAlignment(Qt.AlignCenter)
        self.lbl_title.setStyleSheet("font-size: 52px; font-weight: 900;")  # raddoppio
        ...
        if self.mode == "home":
            if self.show_home:
                ...
                self._azzera_style_base = self._btn_style_3d(AZZ_BASE, AZZ_DARK, text_color="black", font_px=32, pad_v=16, pad_h=28, radius=18)
                alt_base = "#f9e79f"; alt_dark = "#f4d03f"
                self._azzera_style_blink = self._btn_style_3d(alt_base, alt_dark, text_color="black", font_px=32, pad_v=16, pad_h=28, radius=18)
                ...
                self.btn_reset.setStyleSheet(self._btn_style_3d(RESET_BASE, RESET_DARK, font_px=32, pad_v=16, pad_h=28, radius=18))
                self.btn_reset.clicked.connect(self._on_reset_clicked_home)
                right_bar_l.addWidget(self.btn_reset)
            else:
                # Niente Home: Azzera a sinistra (raddoppiato), Reset a destra (raddoppiato)
            ...
            # mode='default': Home a sinistra (normale), Reset a destra (normale)
            self.btn_home = QPushButton("Home", left_bar)
            self.btn_home.setCursor(Qt.PointingHandCursor)
            self.btn_home.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
            self.btn_home.setStyleSheet(self._btn_style_3d(HOME_BASE, HOME_DARK, font_px=16))
    ...
    def _on_reset_clicked_page(self):
        # Reset nelle pagine: callback + fallback reset app/machine, poi Home
